...
const SoupsPage = () => {
  ...
  const handleSearch = async (searchQuery) => {
    ...
    if (searchQuery === "") {
      ...
      fetchSearchByName(category, searchQuery)
        ...
        .catch((error) => {
          toast.error("Неможливо знайти введену вами назву супу.", {
            autoClose: 5000,
  ...
  useEffect(() => {
    fetchTotalRows(category)
      ...
      .catch((error) => {
        console.error("Error fetching soups total items:", error);
      });
  }, []);

  useEffect(() => {
    if (soupsDataCache.has(page)) {
      setSoupsData(soupsDataCache.get(page));
      setLoading(false);
    } else {
      setLoading(true);
      fetchFoodCards(category, page)
        .then((data) => {
          setSoupsDataCache(soupsDataCache.set(page, data));
          setSoupsData(data);
          setLoading(false);
        })
        .catch((error) => {
          console.error("Error fetching soups food cards:", error);
          setLoading(false);
        });
    }
  }, [page, soupsDataCache]);
